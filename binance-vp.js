// ajs- Precise Volume Profile (POC/VAH/VAL) via Binance aggTrades in Zeitfenstern
// Lädt den gesamten UTC-Vortag in Chunks und berechnet Value Area 70% um den POC.
// Minimal-invasiv: füllt nur cPOC/cVAH/cVAL; keine UI/Logik-Änderungen.
(function(){
  ...
  function computeValueArea(bins, coverage=0.7){
    ...
    const arr = bins.slice().sort((a,b)=>a[0]-b[0]); // [price, volUSDT]
  ...
  async function loadPreviousDayHistogram(base, stepMs=5*60*1000, volType="quote"){
    ...
    for(let t=start; t<end; t+=stepMs){
      ...
      for(const [p,v] of arr){
        const key = Number(p);
        const old = bins.get(key) || 0;
        bins.set(key, old + Number(v||0));
      }
    }
  ...
  async function fillPreciseVP(){
    ...
    try{
      const hist = await loadPreviousDayHistogram(m, 5*60*1000, "quote"); // USDT-Volumen
      const {poc, vah, val} = computeValueArea(hist, 0.7);
      if ($("#cPOC") && poc!=null) $("#cPOC").value = Number(poc).toFixed(2);
      if ($("#cVAH") && vah!=null) $("#cVAH").value = Number(vah).toFixed(2);
      if ($("#cVAL") && val!=null) $("#cVAL").value = Number(val).toFixed(2);
      console.log("[ajs] Precise VP bins:", hist.length);
    }catch(e){
      console.error("[ajs] Precise VP failed:", e);
    }
  }

  document.addEventListener("DOMContentLoaded", ()=>{
    $("#market")?.addEventListener("change", ()=> setTimeout(fillPreciseVP, 300));
    setTimeout(fillPreciseVP, 800);
  });
})();
